...
router.post("/", upload.single("logo"), async (request, response) => {
  ...
  if (isSetupComplete) {
    return response
      .status(401)
      .json({ error: "Unauthorized:Setup already complete" });
  } else {
    if (request.file) {
    ...
    await configuration.updateAdminUser(
...
router.post(
  ...
  async (request, response) => {
    ...
    if (request.body.refreshInterval) {
      await configuration.updateRefreshIntervalMinutes(
        request.body.refreshInterval,
      );
...
router.post("/refresh-interval", checkSuperAdmin, async (request, response) => {
  ...
  if (!Number.isFinite(refreshInterval) || refreshInterval < 1) {
    return response
      .status(400)
      .json({ error: "refreshInterval must be a positive integer" });
  }
  ...
  response.json({
    status: 200,
    refreshIntervalMinutes: await configuration.getRefreshIntervalMinutes(),
  });
});
...
router.post("/users", checkSuperAdmin, async (request, response) => {
  ...
  if (!normalizedRole) {
    return response
      .status(400)
      .json({ error: "Role must be superadmin or resource_manager" });
  }
