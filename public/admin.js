...
async function clearResourceIssueState(resourceName) {
  if (!resourceName) return;
  if (!confirm(`Clear user-reported issue state for "${resourceName}"?`)) return;

  try {
    const res = await fetch(`/resources/issue-reports/${encodeURIComponent(resourceName)}`, {
      method: 'DELETE'
    });

    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      alert(data.error || 'Failed to clear reported issue state');
      return;
    }

    alert(`Cleared user-reported issue state for "${resourceName}".`);
  } catch (e) {
    console.error(e);
    alert('Error clearing reported issue state');
  }
}
...
async function loadAnnouncements() {
  ...
  try {
    const res = await fetch('/resources/announcements');
    ...
    if (announcements.length === 0) {
      tbody.innerHTML = '<tr><td class="py-2 pl-2 pr-3 text-sm text-gray-500" colspan="4">No active announcements.</td></tr>';
      return;
    }
    ...
    announcements.forEach((announcement) => {
      ...
      const expiresAt = expiresAtRaw ? new Date(expiresAtRaw).toLocaleString() : '';
      ...
      tr.innerHTML = `
        <td class="py-2 pl-2 pr-3 text-sm text-gray-900 break-words max-w-[420px]">${escapeText(announcement.message)}</td>
        <td class="py-2 px-3 text-sm text-gray-600 whitespace-nowrap">${escapeText(expiresAt)}</td>
        <td class="py-2 px-3 text-sm text-gray-600 whitespace-nowrap">${escapeText(createdBy)}</td>
        <td class="py-2 px-2 text-right text-sm">
          <button type="button" data-id="${announcement.id}" class="revoke-announcement-btn p-1 text-red-500 hover:text-red-600" title="Revoke announcement" aria-label="Revoke announcement">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 inline-block">
    ...
    tbody.querySelectorAll('.revoke-announcement-btn').forEach((button) => {
      button.addEventListener('click', async () => {
        const id = button.getAttribute('data-id');
        if (!id) return;
        if (!confirm('Revoke this announcement?')) return;

        try {
          const revokeRes = await fetch(`/resources/announcements/${encodeURIComponent(id)}`, { method: 'DELETE' });
          if (!revokeRes.ok) {
            const err = await revokeRes.json().catch(() => ({}));
            throw new Error(err.error || 'Failed to revoke announcement');
          }
          await loadAnnouncements();
        } catch (error) {
          console.error(error);
          alert(error.message || 'Failed to revoke announcement');
        }
      });
    });
  } catch (error) {
    console.error(error);
    tbody.innerHTML = '<tr><td class="py-2 pl-2 pr-3 text-sm text-gray-500" colspan="4">Unable to load announcements.</td></tr>';
  }
}

function openAddAnnouncementModal() {
  const messageInput = document.getElementById('modal-announcement-message');
  const expiryInput = document.getElementById('modal-announcement-expiry');
...
async function saveNewAnnouncement() {
  const messageInput = document.getElementById('modal-announcement-message');
  const expiryInput = document.getElementById('modal-announcement-expiry');
  if (!messageInput || !expiryInput) return;
  ...
  try {
    ...
    const res = await fetch('/resources/announcements', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ message, expires_at: expiryValue })
    });
...
function setImportInProgress(inProgress, statusMessage = '') {
  ...
  if (submitBtn) {
    ...
    submitBtn.classList.toggle('opacity-60', inProgress);
    submitBtn.classList.toggle('cursor-not-allowed', inProgress);
    submitBtn.textContent = inProgress ? 'Importing...' : 'Import';
  }

  if (cancelBtn) {
    cancelBtn.disabled = inProgress;
    cancelBtn.classList.toggle('opacity-60', inProgress);
    cancelBtn.classList.toggle('cursor-not-allowed', inProgress);
  }
  ...
  if (status) {
    if (inProgress) {
      ...
      if (statusText) {
        statusText.textContent = statusMessage || 'Import started. Processing CSV...';
      }
    } else {
...
async function importCSV() {
  ...
  try {
    setImportInProgress(true, `Import started. Processing ${data.length} item${data.length === 1 ? '' : 's'}...`);
    ...
    if (res.ok) {
      ...
      const importedCount = result?.imported?.resources || data.length;
      setImportInProgress(true, `Import complete (${importedCount} resources). Loading admin dashboard...`);
