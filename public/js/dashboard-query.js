(function () {
  ...
  const categoryLinks = Array.from(document.querySelectorAll('.category-tag[data-view]'));
  const categorySections = Array.from(document.querySelectorAll('details[data-category-section]'));
  const noResults = document.getElementById('no-results');
  const announcementBanner = document.getElementById('announcement-banner');
  const announcementCards = Array.from(document.querySelectorAll('#announcement-banner-list .announcement-card'));
  const currentIssuesAccordion = document.getElementById('current-issues-accordion');
  const currentIssuesCards = Array.from(document.querySelectorAll('#current-issues-grid .resource-card'));
  ...
  function applyState(state) {
    ...
    const visibleIssueCards = currentIssuesCards.filter((card) => !card.classList.contains('hidden')).length;
    ...
    if (currentIssuesAccordion) {
      currentIssuesAccordion.classList.toggle('hidden', visibleIssueCards === 0 && !!state.search);
    }
