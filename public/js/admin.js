...
function isValidIcmpTarget(value) {
  const target = String(value || "").trim();
  if (!target) return false;
  return !/\s/.test(target);
}

function validateStatusTarget(value, uiCheckType) {
  if (uiCheckType === "icmp-ping") {
    return isValidIcmpTarget(value);
  }
...
function formatErrorTimestamp(value) {
  ...
  const sqliteLike = normalized.match(
...
async function fetchLatestErrors() {
  ...
  return errors.slice().sort((a, b) => Number(b.id || 0) - Number(a.id || 0));
}
...
function closeModal(alias) {
  ...
  setTimeout(() => {
    ...
    modal.classList.remove("exiting");
    content && content.classList.remove("exiting");
    const anyOpen = qsa(".modal-overlay").some(
...
function renderEnhancedTable(tableId, options = {}) {
  ...
  const filtered = rows.filter((row) => {
    ...
    const filterCell = row.children[2]
      ? row.children[2].textContent.trim()
      : "";
    const filterValues = filterCell
      .split(",")
  ...
  if (pages) {
    ...
    buildPageWindow().forEach((pageEntry) => {
      if (pageEntry === "…") {
        ...
        ellipsis.className =
          "inline-flex min-w-[20px] items-center justify-center px-1 text-sm text-gray-500";
      ...
      button.className =
        "table-page-btn rounded border px-2.5 py-1.5 text-sm font-medium";
...
function updateStats() {
  ...
  if (usersCard) {
    ...
    value &&
      (value.textContent = state.isSuperAdmin
...
function renderCategoryFilterOptions() {
  ...
  [
    ...new Set(
      state.resources.flatMap((item) => item.categories || []).filter(Boolean),
    ),
  ]
...
function initializeCredentialRoleOptions() {
  ...
  if (createSelect) {
    ...
    createSelect.innerHTML = options
      ...
      .join("");
    createSelect.value = options.some((option) => option.value === current)
      ? current
      : "";
  }
  ...
  if (editSelect) {
    ...
    editSelect.innerHTML = options
      ...
      .join("");
    editSelect.value = options.some((option) => option.value === current)
      ? current
      : "";
  }
}
...
function renderAnnouncements() {
  ...
  tbody.innerHTML = state.announcements
    .map((announcement) => {
      ...
      const expiresDateTime = announcement.expires_at
        ? toDateTimeLocalValue(announcement.expires_at)
        : "";
      const expiresLabel = announcement.expires_at
        ? new Date(
            String(announcement.expires_at).replace(" ", "T"),
          ).toLocaleString()
        : "";
      const createdBy = announcement.created_by
        ? `${announcement.created_by}${announcement.created_by_role ? ` (${announcement.created_by_role})` : ""}`
        : "-";
...
async function fetchAndRenderErrors() {
  state.errors = await fetchLatestErrors();
  renderErrors();
  initEnhancedTables();
  refreshEmptyStates();
}
...
async function loadAllData() {
  ...
  state.resources = Array.from(resourceMap.values()).sort((a, b) =>
  ...
  if (state.isSuperAdmin) {
    ...
    state.errors =
      errorsRes.status === "fulfilled" ? errorsRes.value || [] : [];
  } else {
    state.users = [];
    const errorsRes = await Promise.allSettled([fetchLatestErrors()]);
    state.errors =
      errorsRes[0].status === "fulfilled" ? errorsRes[0].value || [] : [];
  }
...
function updateResourceCheckSections() {
  ...
  const createUrl = qs("#resource-url");
  const createUrlLabel = qs('label[for="resource-url"]');
  if (createType && createScrape && createApi) {
    createScrape.style.display =
      createType.value === "web-scraping" ? "block" : "none";
    createApi.style.display =
      createType.value === "api-check" ? "block" : "none";
    if (createUrl) {
      createUrl.placeholder =
        createType.value === "icmp-ping"
          ? "e.g., 8.8.8.8 or status.example.org"
          : "https://status.example.org/resource";
    }
    if (createUrlLabel) {
      createUrlLabel.textContent =
        createType.value === "icmp-ping" ? "Status Target" : "Status URL";
    }
  }
  ...
  const editUrl = qs("#edit-resource-url");
  const editUrlLabel = qs('label[for="edit-resource-url"]');
  if (editType && editScrape && editApi) {
    editScrape.style.display =
      editType.value === "web-scraping" ? "block" : "none";
    editApi.style.display = editType.value === "api-check" ? "block" : "none";
    if (editUrl) {
      editUrl.placeholder =
        editType.value === "icmp-ping"
          ? "e.g., 8.8.8.8 or status.example.org"
          : "https://status.example.org/resource";
    }
    if (editUrlLabel) {
      editUrlLabel.textContent =
        editType.value === "icmp-ping" ? "Status Target" : "Status URL";
    }
  }
}
...
function bindModalTriggers() {
  ...
  document.addEventListener("keydown", (event) => {
    ...
    const open = qsa(".modal-overlay").find(
...
function resetApiExplorerState() {
  ...
  if (preview) {
    preview.innerHTML =
      '<p class="rounded border border-gray-200 bg-white px-3 py-2 text-xs text-gray-500">No API data loaded yet. Enter a URL and click Analyze.</p>';
  }
...
function getApiExplorerSearchInputs() {
  ...
  const keyInput =
    qs("#api-explorer-search-key", modal) || allSearchInputs[0] || null;
  const valueInput =
...
function filterApiExplorerData(node, keyTerm, valueTerm, nodeKey = "") {
  ...
  if (Array.isArray(node)) {
    ...
    node.forEach((item, index) => {
      const result = filterApiExplorerData(
        item,
        keyTerm,
        valueTerm,
        String(index),
...
function applyApiExplorerFilters({ preserveSelection = false } = {}) {
  ...
  const { keyInput, valueInput } = getApiExplorerSearchInputs();
  const keyTerm = String(keyInput?.value || "")
    .trim()
    .toLowerCase();
  const valueTerm = String(valueInput?.value || "")
    .trim()
    .toLowerCase();

  const filteredResult = filterApiExplorerData(
    state.apiExplorerData,
    keyTerm,
    valueTerm,
  );
  const filteredData = filteredResult.includeWholeNode
    ? state.apiExplorerData
    : filteredResult.value;
  if (!hasApiExplorerResults(filteredData)) {
    preview.innerHTML =
      '<p class="rounded border border-gray-200 bg-white px-3 py-2 text-xs text-gray-500">No matching fields found.</p>';
    state.apiSelectedField = null;
    setApiSelectedFieldPreview();
    setApiUseFieldEnabled(false);
    return;
...
function initApiExplorer() {
  ...
  useFieldButton &&
    useFieldButton.addEventListener("click", () => {
  ...
  document.addEventListener("click", (event) => {
    ...
    qsa(".api-field-row").forEach((item) =>
    ...
    return entries
      .map(([key, child]) => {
        ...
        if (child && typeof child === "object") {
          ...
          const meta = Array.isArray(child)
            ? `Array[${child.length}]`
            : "Object";
          return `
...
async function handleDelete(type, button) {
  ...
  if (type === "resource") {
    ...
    await Promise.all(
      categories.map((category) =>
        requestJson(
          `/resources/category/${encodeURIComponent(category)}/${encodeURIComponent(resourceName)}`,
          { method: "DELETE" },
        ),
      ),
    );
  ...
  if (type === "announcement") {
    ...
    await requestJson(`/resources/announcements/${encodeURIComponent(id)}`, {
      method: "DELETE",
    });
    return;
  }
  ...
  if (type === "credential") {
    ...
    await requestJson(`/setup/users/${encodeURIComponent(username)}`, {
      method: "DELETE",
    });
    return;
  }
  ...
  if (type === "error") {
    ...
    await requestJson(`/resources/errors/${encodeURIComponent(id)}`, {
      method: "DELETE",
    });
  }
}
  ...
  document.addEventListener("click", async (event) => {
    ...
    if (editButton) {
      ...
      if (editType === "category") {
        qs("#edit-category-name").value =
          editButton.getAttribute("data-edit-name") || "";
        state.editContext = {
          type: "category",
      ...
      if (editType === "resource") {
        qs("#edit-resource-name").value =
          editButton.getAttribute("data-edit-name") || "";
        qs("#edit-resource-url").value =
          editButton.getAttribute("data-edit-url") || "";
        qs("#edit-resource-favicon-url").value =
          editButton.getAttribute("data-edit-favicon") || "";
        if (qs("#edit-resource-category-options")) {
          let parsedCategories = [];
          const rawCategories =
            editButton.getAttribute("data-edit-categories") || "";
          if (rawCategories) {
            try {
          ...
          if (parsedCategories.length === 0) {
            const fallback =
              editButton.getAttribute("data-edit-category") || "";
            if (fallback) parsedCategories = [fallback];
          }

          setCheckedValues("#edit-resource-category-options", parsedCategories);
        }
        qs("#edit-resource-check-type").value = toUiCheckType(
          editButton.getAttribute("data-edit-check-type") || "api",
        );
        qs("#edit-resource-scrape-keywords").value =
          editButton.getAttribute("data-edit-scrape-keywords") || "";
      ...
      if (editType === "announcement") {
        qs("#edit-announcement-message").value =
          editButton.getAttribute("data-edit-message") || "";
        qs("#edit-announcement-expires").value =
          editButton.getAttribute("data-edit-expires") || "";
        qs("#edit-announcement-type").value = toAnnouncementType(
          editButton.getAttribute("data-edit-announcement-type") ||
            "informative",
        );
        state.editContext = {
          type: "announcement",
      ...
      if (editType === "credential") {
        const selectedRole =
          editButton.getAttribute("data-edit-role") || "Resource Manager";
        qs("#edit-user-username").value =
          editButton.getAttribute("data-edit-username") || "";
    ...
    if (clearIssue) {
      ...
      try {
        await requestJson(
          `/resources/issue-reports/${encodeURIComponent(name)}`,
          { method: "DELETE" },
        );
    ...
    const clearErrors = event.target.closest(
      'button[aria-label="Delete all error logs"]',
    );
    ...
    const refreshErrors = event.target.closest(
      'button[data-action="refresh-errors"], button[aria-label="Manual refresh error log"]',
    );
    if (refreshErrors) {
    ...
    if (
      exportButton &&
      exportButton.textContent.trim().toLowerCase() === "export"
    ) {
...
function bindForms() {
  ...
  formResources &&
    formResources.addEventListener("submit", async (event) => {
      ...
      if (!name || !url || categories.length === 0)
        return alert("Name, URL, and at least one category are required");
      if (!checkTypeValue) return alert("Check type is required");
      if (!validateStatusTarget(url, checkTypeValue)) {
        return alert(
          checkTypeValue === "icmp-ping"
            ? "For ICMP Ping, enter a valid host or IP address"
            : "Status URL must be a valid http(s) URL",
        );
      }
      if (faviconUrl && !isValidAbsoluteUrl(faviconUrl)) {
        return alert("Favicon URL must be a valid http(s) URL");
      }
  ...
  formEditCategories &&
    formEditCategories.addEventListener("submit", async (event) => {
      event.preventDefault();
      ...
      try {
        await requestJson(
          `/resources/category/${encodeURIComponent(state.editContext.oldName)}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
  ...
  formEditResources &&
    formEditResources.addEventListener("submit", async (event) => {
      ...
      const newFaviconUrl = qs("#edit-resource-favicon-url").value.trim();
      const newCategories = getSelectedValues(
        "#edit-resource-category-options",
      );
      ...
      if (!newName || !newUrl || newCategories.length === 0)
        return alert("Name, URL, and at least one category are required");
      if (!checkTypeValue) return alert("Check type is required");
      if (!validateStatusTarget(newUrl, checkTypeValue)) {
        return alert(
          checkTypeValue === "icmp-ping"
            ? "For ICMP Ping, enter a valid host or IP address"
            : "Status URL must be a valid http(s) URL",
      ...
      if (newFaviconUrl && !isValidAbsoluteUrl(newFaviconUrl)) {
        return alert("Favicon URL must be a valid http(s) URL");
      }
      ...
      const payload = {
        ...
        status_page: newUrl,
        favicon_url: newFaviconUrl || null,
  ...
  formAnnouncements &&
    formAnnouncements.addEventListener("submit", async (event) => {
      event.preventDefault();
      ...
      if (!message || !expiresAt)
        return alert("Message and expiration date/time are required");
  ...
  formEditAnnouncements &&
    formEditAnnouncements.addEventListener("submit", async (event) => {
      event.preventDefault();
      if (!state.editContext || state.editContext.type !== "announcement")
        return;
      ...
      try {
        await requestJson(
          `/resources/announcements/${encodeURIComponent(state.editContext.id)}`,
          { method: "DELETE" },
        );
        await requestJson("/resources/announcements", {
          method: "POST",
  ...
  formCredentials &&
    formCredentials.addEventListener("submit", async (event) => {
      event.preventDefault();
  ...
  formEditCredentials &&
    formEditCredentials.addEventListener("submit", async (event) => {
      event.preventDefault();
      ...
      if (isSuperadminEdit) {
        try {
          ...
          const schoolName =
            qs("#school\\.name")?.value?.trim() ||
            document.body.dataset.schoolName ||
            "";
          const refreshInterval =
            qs("#status-check-interval")?.value ||
            document.body.dataset.refreshInterval ||
            "30";
          ...
          if (!response.ok)
            throw new Error("Failed to update superadmin credentials");
      ...
      try {
        await requestJson(
          `/setup/users/${encodeURIComponent(state.editContext.oldUsername)}`,
          { method: "DELETE" },
        );
        await requestJson("/setup/users", {
          method: "POST",
  ...
  formBulkImport &&
    formBulkImport.addEventListener("submit", async (event) => {
      event.preventDefault();
      ...
      try {
        ...
        while (!finished) {
          ...
          const progress = await requestJson(
            `/resources/import/progress/${encodeURIComponent(jobId)}`,
          );
...
async function init() {
  ...
  state.isSuperAdmin =
    document.body.dataset.isSuperadmin === "true" ||
  ...
  createCheckType &&
    createCheckType.addEventListener("change", updateResourceCheckSections);
  editCheckType &&
    editCheckType.addEventListener("change", updateResourceCheckSections);
